...
<script>
function totallist(){
	location.href='debateList.do?current_page=1';
}
function signuplist(){
	location.href='debateSignUpList.do?current_page=1';
}
function savingslist(){
	location.href='debateSavingsList.do?current_page=1';
}
function etclist(){
	location.href='debateEtcList.do?current_page=1';
}
</script>
</head>
<!-- *********************** 게시판 글쓰기 폼 ****************************  -->	
	...
	<tr>
		<td>
				<div style="float: left; width: 40%; padding:10px;" >
					...
					<button type="button" onclick="javascript:signuplist()">예금토론</button>
					<button type="button" onclick="javascript:savingslist()">적금토론</button>
					<button type="button" onclick="javascript:etclist()">기타</button>
				</div>
				<div style="float: right; width: 50%; vertical-align: center">
					<form name=searchf method=post action="debateSearch.do">
	...
	<c:forEach var="board" items="${boardList}">
		<tr>
			...
			<td width="320">
				<p align="center">
					<a
						href="debateView.do?idx=${board.getIdx()}
...
<table cellspacing="1" width="700">
	<tr>
		<td><input type="button" value="글쓰기"
			onclick="window.location='debate_show_write_form.do'"></td>
	</tr>
</table>
<%-- </c:if> --%>
</div>	
